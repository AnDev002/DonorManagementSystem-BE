...
export class OrderService {
  ...
  async getSellerOrders(userId: string, status?: string) {
    ...
    if (!shop) return []; // Return rỗng nếu chưa có shop, tránh lỗi crash query
    ...
    return this.prisma.order.findMany({
      where: {
        items: {
          some: {
            product: { shopId: shop.id } // Lọc đơn hàng có chứa SP của shop
          }
        },
      ...
      include: {
        ...
        items: { 
          // [QUAN TRỌNG] Chỉ include items thuộc về Shop này để Frontend lấy items[0] luôn đúng
          where: {
