...
export class AdminUsersService {
  ...
  async deleteUser(adminId: string, userId: string) {
    ...
    const user = await this.prisma.user.findUnique({ 
        where: { id: userId },
        include: { shop: true } // Include Shop để check
    });
    ...
    // Lưu ý: Nếu user đã có Đơn hàng (Order), Prisma có thể báo lỗi Foreign Key nếu schema không để onDelete: Cascade.
    // Ở đây tôi giả định xóa User để test, dữ liệu order chưa quan trọng hoặc schema đã handle.
    await this.prisma.$transaction(async (prisma) => {
        // A. Nếu là Seller, xóa Shop trước (nếu có)
        if (user.role === 'SELLER' && user.shop) {
             // Xóa các sản phẩm của shop này (nếu cần thiết)
             // await prisma.product.deleteMany({ where: { shopId: user.shop.id } });
        ...
        // B. Xóa Cart và PointWallet (thường là 1-1, nên xóa tay hoặc cascade)
    ...
    return { message: `Đã xóa vĩnh viễn tài khoản ${user.email}` };
  }
}